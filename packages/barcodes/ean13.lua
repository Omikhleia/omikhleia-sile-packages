...
local function ean13 (text)
  ...
  -- half of the code
  local selector = tableCode[tonumber(text[1]) + 1]
  for i = 2, 7 do
...
local function ean13addOn2 (text)
  ...
  for i = 1, 2 do
    local selectedTable = selector[i]
    local digit = tonumber(text[i]) + 1
    local pat = tableNumberBars[selectedTable][digit]
    pattern = pattern .. pat
    if i < 2 then
...
local function ean13addOn5 (text)
  ...
  local V = (tonumber(text[1]) + tonumber(text[3]) + tonumber(text[5])) * 3 + (tonumber(text[2]) + tonumber(text[4])) * 9
  local Z = math.floor(V / 10) * 10
  local VU = V - Z -- i.e. the unit's position in V
    ...
    if SU.boolean(options.showdigits, true) then
      ...
      SILE.call("font", { family = SILE.scratch.ean13.font.family, size = SILE.scratch.ean13.font.size * X }, function ()
        ...
        -- So we get back to the start of the barcode.
          ...
          -- We add a 0.5X displacement from the normal guard (ending in a bar,
          -- while the centralbar starts with a space)
...
SILE.registerCommand("ean13:addon", function (options, _)
    ...
    if SU.boolean(options.showdigits, true) then
      ...
      SILE.call("font", { family = SILE.scratch.ean13.font.family, size = SILE.scratch.ean13.font.size * X }, function()
        SILE.call("raise", { height = (H - 4) * X }, function() -- 0.5X minimum between character and bars,
                                                                -- but it looks much better looking with a full 1X
          SILE.call("kern", { width = -9 * #code * X })
          for i = 1, #code do
            local h = SILE.call("hbox", {}, { code[i] }) -- first digit, at the start of the Add-ons Left Quiet Zone
            h.width = SILE.length()
...
SILE.registerCommand("ean13:font", function (options, _)
...
\script[src=packages.barcodes.ean13]
...
The \autodoc:command{\ean13:font[family=<family>]} allows setting the font family used
for the human readable interpretation, would the above be unavalaible to you or another
choice be preferred. Obviously, a monospace font is strongly recommended.
