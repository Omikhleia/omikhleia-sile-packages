...
SILE.registerCommand("omipoetry:prosody", function (options, content)
  ...
  prosodyBox.height = SILE.settings.get("omipoetry.lineheight")
  SILE.call("rebox", {height=0, width=0}, function()
    SILE.call("raise", { height = SILE.settings.get("omipoetry.offset"):absolute() + vadjust:tonumber()}, function ()
...
SILE.registerCommand("omipoetry:prosodyfont", function (_, content)
...
SILE.settings.declare({
    parameter = "omipoetry.lineheight",
    type = "length",
...
SILE.registerCommand("omipoetry:v", function (options, content)
  local n = options.n and SU.cast("integer", options.n)
  SILE.typesetter:leaveHmode()
  if n then
    typesetVerseNumber(""..n)
  end
  SILE.process(inputfilter.transformContent(content, prosodyFilter, { mode = options.mode }))
  SILE.typesetter:leaveHmode()
  SILE.typesetter:pushVglue(SILE.settings.get("document.parskip"))
...
named \doc:code{experimental} is also proposed, using an en-dash and a small, slightly lowered, half-circle,
respectively.
...
Alternately, the \doc:code{mixed} mode is a kind of compromise, using an en-dash for the macron, but keeping
the (lowered) breve.
